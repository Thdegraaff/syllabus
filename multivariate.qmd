...
A final remark concerns a peculiarity of `STATA`. In the regression output of above, `STATA` does not provide the $\bar{\text{R}}^2$. That is because of the option `, robust`. Without that option, the regression output would give both measures of fit. 

```{stata}
reg testscr str el_pct
```

Another option is to specifically ask `STATA` to display the $\bar{\text{R}}^2$ by invoking the command `display`, then some text (text always goes between strings), and finally the thing you want to see (`e(r2_a)`). Something like:
...
```{r fig.align="center", echo=FALSE, fig.cap = "A linear relation", label='fig-scatterlfitcaschool', out.width="600px"}
...
```{r fig.align='center', echo=FALSE, fig.cap = "A non-linear relation", label='fig-scatterincome', out.width="600px"}
...
Note by the way the syntax `c.avginc##c.avginc` which seems a bit strange. However, this particular line of code is very useful for later tabulation, plotting and other manipulations of the output. In this way `STATA` knows that there should be a quadratic effect of the same variable (`avginc`). The syntax `c.` denotes that the variable should be considered as continuous instead of as an integer (try using without `c.` and behold the horrible output). There are four useful operators that you want to know when working with polynomials and interaction effects:

- `i.` operator: this specifies that the following variable is an integer and should be considered on all its level. This actually create indicator or dummies variables
...
- `##` binary operator that specifies both the interaction effect between two variables and the individual variable effects
...
```{r fig.align='center', echo=FALSE, fig.cap = "A non-linear relation", label='fig-scatterqua', out.width="600px"}
...
```{stata, collectcode=TRUE, echo = 1:4}
gen hiel = (el_pct >= 10)
reg testscr c.str##i.hiel, r
margins hiel, at(str=( 14 ( 2 ) 26 ))
marginsplot
quietly graph export figures/interactionplot.png, replace
```
...
```{r fig.align='center', echo=FALSE, fig.cap = "Predicted population regression lines of districts with large and small percentage english learners", label='fig-interactionplot', out.width="600px"}
...
```{r fig.align='center', echo=FALSE, fig.cap = "A non-linear relation", label='fig-scatterlnincome', out.width="600px"}
...
```{r fig.align='center', echo=FALSE, fig.cap = "A non-linear relation", label='fig-scattercompare', out.width="600px"}
...
```{stata readfatalitydata, collectcode=TRUE, results="hide"}
use "./data/fatality.dta", clear
gen fatality = allmort/pop * 10000
```
and then run a simple regression:
```{stata official, cleanlog=FALSE, echo=FALSE, collectcode=TRUE}
