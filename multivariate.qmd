...
2. The root mean square error (RMSE) which denotes as well the stdandard deviation of $\hat{u}_i$ but now without degrees of freedom. We have seen this before in Eq. \@ref(eq:rmse) and does not change. 
...
2.  The second least squares assumption is more or less as before but now in a multivariate fashion, so the whole set of (X$_{1i}$,\ldots,X$_{ki}$,Y$_i$), with $i =1,\ldots,n$, should be independent and identical distributed ($i.i.d$).
...
```{r fig.align="center", echo=FALSE, fig.cap = "A linear relation", label='scatterlfitcaschool', out.width="600px"}
...
```{r fig.align='center', echo=FALSE, fig.cap = "A non-linear relation", label='scatterincome', out.width="600px"}
