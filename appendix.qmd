...
Random variables are governed by *distribution functions* which are mathematical function that
...
```{r, echo=FALSE, fig.cap = "The correlation coefficient and the relation between observed $x$ and $y$", label='corrcoef'}
...
\end{equation}
which is a very important concept in applied economics as it resembles two groups of which one received *treatment* and the other one not. Other examples of the use of conditional means: difference in wages among gender (glass ceiling) and mortality rate differences between those who are treated and those who are. Now if $E(X|Z)$ is constant, then $corr(X,Z) = 0$. We then say that $X$ and $Z$ are independent. 

## Sampling in frequentist statistics {#secssampling}
...
```{r, echo=FALSE, fig.cap = "Sampling distribution when you throw a coin two times", label='coin1'}
...
```{r, echo=FALSE, fig.cap = "Sampling distribution when you throw a coin 100 times", label='coin100'}
...
2. **The Central Limit Theorem**: when independent random variables are summed up, their properly normalized sum tends toward a normal distribution even if the original variables themselves are not normally distributed. So $\bar{Y}$ is approximately distributed $N(\mu_Y,\frac{\sigma^2_Y}{n})$
